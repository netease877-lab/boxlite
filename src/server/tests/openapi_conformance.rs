...
/// Load a YAML spec from the openapi/ directory and convert to JSON Value.
fn load_yaml_spec_file(filename: &str) -> Value {
...
/// Load the hand-written sandbox REST API spec.
fn load_yaml_spec() -> Value {
    load_yaml_spec_file("rest-sandbox-open-api.yaml")
}

/// Load the coordinator admin API spec.
fn load_coordinator_spec() -> Value {
    load_yaml_spec_file("coordinator-open-api.yaml")
}
...
// ============================================================================
// Coordinator Admin API conformance
// ============================================================================

#[test]
fn test_coordinator_info_metadata() {
    let actual = utoipa_spec();
    let expected = load_coordinator_spec();

    // Coordinator spec version should match utoipa
    assert_eq!(
        actual["info"]["version"].as_str().unwrap(),
        expected["info"]["version"].as_str().unwrap(),
        "Version mismatch"
...
#[test]
fn test_coordinator_paths_exist() {
    let actual = utoipa_spec();
    let expected = load_coordinator_spec();

    let yaml_paths = expected["paths"]
        .as_object()
        .expect("Coordinator YAML has no paths");
    let utoipa_paths = actual["paths"].as_object().expect("utoipa has no paths");

    let mut missing = Vec::new();
    for yaml_path in yaml_paths.keys() {
        if !utoipa_paths.contains_key(yaml_path.as_str()) {
            missing.push(yaml_path.clone());
        }
    }
    ...
    assert!(
        missing.is_empty(),
        "Missing coordinator paths in utoipa spec: {missing:?}"
    );
}

#[test]
fn test_coordinator_operations_exist() {
    let actual = utoipa_spec();
    let expected = load_coordinator_spec();
    ...
    for (path, yaml_ops) in yaml_paths {
        let yaml_ops = yaml_ops.as_object().unwrap();
        let utoipa_ops = match utoipa_paths.get(path.as_str()) {
            Some(v) => v.as_object().unwrap(),
            None => continue,
        };
    ...
    assert!(
        missing_ops.is_empty(),
        "Missing coordinator operations in utoipa spec: {missing_ops:?}"
    );
}

#[test]
fn test_coordinator_schemas_exist() {
    let actual = utoipa_spec();
    let expected = load_coordinator_spec();

    let yaml_schemas = expected["components"]["schemas"]
        .as_object()
        .expect("Coordinator YAML has no schemas");
    let utoipa_schemas = actual["components"]["schemas"]
        .as_object()
        .expect("utoipa has no schemas");
    ...
    assert!(
        missing.is_empty(),
        "Missing coordinator schemas in utoipa spec: {missing:?}"
    );
}
